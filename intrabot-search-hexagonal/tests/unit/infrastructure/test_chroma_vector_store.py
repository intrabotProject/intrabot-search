...
def populated_vector_store() -> ChromaVectorStore:
    ...
    store._collection.add(
        ...
        documents=[f.text for f in ALL_CHUNK_FIXTURES],
        metadatas=[f.metadata for f in ALL_CHUNK_FIXTURES],
    )
    return store
...
class TestChromaVectorStore:
    ...
    def test_document_filename_resolved_from_source_metadata_key(
        ...
        """
        results = populated_vector_store.query_similar_chunks(QUERY_EMBEDDING_TELEWORK, 1)
        assert results[0].document_filename == CHUNK_TELEWORK_POLICY.source  # .source (not .document_filename)
